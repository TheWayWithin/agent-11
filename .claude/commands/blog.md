---
name: blog
description: Draft a voice-aligned blog post (plus X thread, LinkedIn post, WIP line and image specs) on any topic, pulling context from the repo
---
...
1. **Resolve the voice guide** — same chain as `/dailyreport`, ending at the `jamie-voice`
   skill, which is the voice authority
2. **Resolve the staging directory** — the synced vault drafts folder by default
3. **Gather context** — read `README.md`, `CLAUDE.md`, the brief file (if given), any
...
Five files per package, all in the same directory (jpub finds companions beside the post):

```
~/shared/content/drafts/
├── 2026-04-11-feature-flags-small-teams.md            # The post
├── 2026-04-11-feature-flags-small-teams-twitter.md    # X thread
├── 2026-04-11-feature-flags-small-teams-linkedin.md   # LinkedIn post
├── 2026-04-11-feature-flags-small-teams-wip.md        # WIP.co line
└── 2026-04-11-feature-flags-small-teams-images.md     # Image specs
```

File naming: `YYYY-MM-DD-slug.md` where slug is derived from the title (lowercased,
hyphenated, 3-6 words). The five files must sit in the **same** directory: jpub discovers
the `-twitter`, `-linkedin` and `-wip` companions in the post file's own directory.
...
3. `blog/` inside the current repo, with a printed warning
...
`/blog` uses the same voice guide system as `/dailyreport`. Resolution order (first hit
wins):
...
Read these env vars with Bash calls (use `echo` so an unset var returns empty):
...
### Step 4: Title the piece through `jamie-titles`
...
The slug is derived from the chosen title: lowercase, hyphenated, 3-6 words, stop words
removed, capped at 60 characters.

### Step 5: Draft the long-form post
...
- **Length**: 700-1500 words. Shorter if the topic doesn't justify more. Respect the
  reader's time.
...
- **No author-bio or contact footer in the body.** The site template renders the
  canonical bio on every post; one in the body duplicates it.
...
- Open with the most arresting line of the piece — a counter-intuitive claim, a surprising
  observation, a blunt finding. Not preamble. The first 140 characters (what shows before
...
ceiling, not a quota. Cut any inline image that does not inform.

**Format:** the hero ships as a raster PNG, roughly 16:9 (~1200×675), under ~300KB. Never
reference a `.svg` from frontmatter; drawing the hero as SVG and rendering it to PNG is
fine. The hero goes in frontmatter (`image:` + `imageAlt:`); inline images are referenced
in the body as `![alt](/images/blog/post-slug-N.png)` at their placement points. The
`/images/blog/` prefix is mandatory — `jpub` resolves images through that fixed prefix and
any other prefix uploads nothing, silently.
...
Write to the staging directory resolved in Step 1.5, creating it if missing.
...
- **`editorialType`** — `essay` for a curated argument or reflection, `build-log` for a
  build-in-public field report. `jpub` defaults a missing value to `build-log`.
...
or verified. If one is missing, write `[VERIFY: what is missing]` rather than an estimate or
a plausible round figure. This binds hardest on the title, the dek and the opening line,
which get quoted without their surrounding text.
...
- Any sentence that is clever before it is clear. Clear wins. Apply the pub test: would
  the author say this out loud to a friend and be understood without explaining?

**URL consistency check** — the URL in every social file must use the *slug only*, not the
file name. The `<slug>` is the value of the `slug:` field in the post's frontmatter (no date
...
frontmatter. Never emit a `/blog/<slug>` URL: that route is retired.
...
### Step 12: Report, and hand over the blog command ONLY
...
✅ Post: <staging-dir>/YYYY-MM-DD-slug.md (<word count> words)
  ...
  🚢 WIP.co: <staging-dir>/YYYY-MM-DD-slug-wip.md (<char count>, <hashtag>)
...
# when ~/shared/content/ exists, then to a repo-local blog/ directory.
CONTENT_DRAFTS_DIR=/path/to/drafts

# Base URL for post links in social posts (shared with /dailyreport).
...
# Product URL for the optional "Try it:" line (e.g. modeloptix.com). If unset,
# the line is omitted rather than leaving a placeholder.
PRODUCT_URL=yourdomain.com

# WIP.co project hashtag (shared with /dailyreport). Only set this when the
# repo's posts genuinely are about that product — see Step 8.
...
Both commands write in the same voice and produce the same five-file package. The
...
- If `DAILYREPORT_VOICE_GUIDE` is set but points at a missing file, you'll see a warning
  and a fall-through to the next step in the chain
...
- **`/dailyreport`** — same voice guide, same package shape, different input
- **`/pmd`** — run `/pmd` first for root cause analysis, then `/blog` to turn the
  post-mortem into a public-facing story
- **`jamie-titles`** — owns the title rubric (Step 4)
- **`jamie-voice`** — the voice authority (Step 2)
- **`jamie-publish`** — owns the publish mechanics and the same blog-first gate

---

*`/blog` turns any topic into a voice-aligned post plus the matching X thread, LinkedIn
post, WIP line and image specs, staged where the publish tool can reach them. No Python
script, no OpenAI key — pure Claude.*
