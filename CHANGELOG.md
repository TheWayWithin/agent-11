...
## [6.3.0] - 2026-08-05 - Map-First Orientation & Honest Enforcement
...
- **Four repo checks**, each silent and exit 0 when compliant: `scripts/validate-sprint6-closeout.sh` (orientation coverage, gate surface intact, decision memo records a verdict per task), `scripts/validate-enforcement-claims.sh` (no document claims protection the rules do not provide), `scripts/validate-deployment-coverage.sh` (install.sh's two mission lists **and** `/coord`'s routing table all match the library), `scripts/validate-bash-route-claims.sh` (nothing describes the Bash gate guard as closing the Bash route, and the three files that carry its description still state the limit).
- **`/coord` routes all 18 executable missions.** `architecture`, `product-description`, `operation-genesis`, `connect-mcp` and `operation-recon` installed correctly and then hard-errored the moment anyone invoked them, because the routing table listed 13 of 18 (A11-ISS-17). The table now carries an explicit **File** column — the command no longer derives a filename from the mission name, which was unguessable given `build` → `mission-build.md` but `dev-setup` → `dev-setup.md`. `validate-deployment-coverage.sh` fails if the table, the library and the installer ever disagree again.
- **The coordinator's phase counters live on disk** (`.claude/scripts/mission-state.py` → `.claude/state/mission-state.json`). `cycles_this_phase`, `clean_rounds` and the per-phase error budget were previously tracked in the orchestrator's head and reported from memory. A counter the model narrates is a claim; a counter on disk is a fact. `mission-state.py cycle` exits 3 when the budget is spent, so escalation no longer depends on the coordinator having counted correctly, and `clean-round` refuses to record a round without `--evidence`. `mission-state.py resume` prints the last phase whose gate passed on evidence, giving a mission that dies mid-phase a recovery point. It is bookkeeping, not enforcement: `.claude/state/` is not a gate path and the script cannot tell a real evidence string from an invented one.
- **The coordinator can delegate a genuine fan-out phase to a dynamic workflow.** It is not rewritten as one — that decision stands, and the reasons are in `sprints/sprint-6-workflows-decision.md`. A new FAN-OUT DELEGATION PROTOCOL section gives it a three-part test a phase must pass before it is eligible, and the constraints that travel with any workflow it invokes: the verify stage runs a different model to the finding stage, nothing auto-merges, findings are hypotheses, and the run reports its real cost. On the shipped evidence 2 of 18 missions have a fan-out-shaped phase and neither is fan-out-shaped whole, so the honest default is that a phase is not eligible.
...
- **The Bash gate guard carries 13 detection branches.** Counted as branches in the guard's own source, so the number and the code cannot drift apart — it was previously stated on two different bases in different files, which is the same defect as overclaiming, pointed the other way. Two steps got it here, both unreleased: v6.2.0 shipped 5 command forms wired inline in the settings template with no script; extracting it to `gate-guard.sh` grew it to 8 branches; today took it to 13. Added by the extraction: `rm`/`truncate`/`shred`/`unlink`, `dd of=`, `ln -s`, in-place `perl`/`ruby`, and then for A11-ISS-16 interpreter one-liners that name a gate path literally alongside a write verb (`python3 -c "open('.quality-gates.json','w')"`), gate paths held in a shell variable and written through it (`G=gates/x; echo y > $G`), `patch`/`git apply`, and `git checkout`/`restore`/`rm`/`mv`. Deleting or reverting a gate passes it as effectively as lowering it, and neither was covered at all. Ordinary Bash is unaffected, regression-tested against `npm test`, `git status gates/`, `rm -rf node_modules`, `python3 gates/run-gates.py`, reading a gate through an interpreter, and assigning a gate path to a variable that is never written through.
- **The guard is no longer described as "closing the Bash route", anywhere.** It narrows it. A path assembled at runtime, an interpreter reaching the path indirectly, anything eval'd or base64-decoded, and any write done by a program the command launches all pass straight through, and no shell hook can catch those. Treat it as a speed bump; the enforceable guarantee is the `Edit()` deny rules. Three surviving instances of the old wording were removed this release, and `validate-bash-route-claims.sh` now fails on both the return of the false claim **and** the deletion of the honest one — an absence-only check passes on an empty file. Tracked as A11-ISS-16.
...
- **Coordinator phase-gated meta-loop + Bash gate guard (Sprint 6c)** — `/coord continue` is now a disciplined outer loop: it **converges** on two clean verify rounds rather than a fixed count, spends a **per-phase error budget** (default 3 cycles) then **escalates to the human instead of burning forward**, takes **condensed 1000–2000 token returns** from specialists (detail to disk, orchestrator context stays clean), flags **unanimous agreement** as a possible correlated-bias signal, and **restarts from the last evidence-passed gate** (never re-running verified-complete phases). Also closes a gap in the 6a read-only enforcement: the `permissions.deny` rules cover Edit/Write/MultiEdit but not Bash, so a new blocking **PreToolUse "read-only gate guard" hook** refuses the common Bash write forms (`>`, `tee`, `sed -i`, `cp`, `mv`, and since the Sprint 6 ship also `rm`, `truncate`, `shred`, `unlink`, `dd of=`, `ln -s`, `perl`/`ruby -i`) against gate paths. It does not close the Bash route: interpreter-mediated writes and variable-held paths pass through (A11-ISS-16). Honest framing: loops are watched-first and human-merged; the error-budget default awaits tuning from a harness-measured token cost.
- **Ratchet loops (Sprint 6b)** — `mission-optimize` is rewritten as the Karpathy ratchet: it isolates work in a git worktree, sets a median-of-3 noise-floor baseline, makes ONE change on a NAMED surface, re-measures, and **keeps the change only if it beats the baseline — otherwise hard-reverts.** Every attempt is logged to `.loops/` as JSONL; runs are capped (10 attempts / 1h / 1000-line diff / token ceiling); nothing auto-merges — every kept change is a hypothesis the human merges. A new deployable **`code-review-loop` skill** applies the same discipline to review: a read-only critic raises evidence-backed findings, a read-write fixer addresses only those findings, re-audit until two clean rounds or a cap. New `loop-discipline-guide.md` (the five-gate "loop or not" test) and a `mission-optimize` input template ship with it. Built on the 6a read-only foundation — the metric, gate, and critic that judge the work are unwritable by the agent doing it. _(Implementation landed; watched validation run + public README/website write-up in the Sprint 6 consolidation pass.)_
- **Read-only quality gates (Sprint 6a)** — the criteria that judge an agent's work are now unwritable by that agent. `library/settings.json.template` ships a `permissions.deny` block making `.quality-gates.json` and any `gates/` directory read-only to every deployed agent, enforced at the tool layer (not a prompt convention). Stops the reward-hacking failure where an agent loosens a threshold instead of doing the work. Note the boundary: the four shipped rules match gate paths, so a test living elsewhere that serves as acceptance criteria is prohibited by instruction but not refused by the tool layer. Paired with a **default-fail verification contract** in tester, developer, and coordinator: every success criterion starts failing and flips to pass only on captured command output — an asserted "done" with no evidence is treated as a failure. _(Implementation landed; full public README/website write-up deferred to the Sprint 6 consolidation pass.)_
