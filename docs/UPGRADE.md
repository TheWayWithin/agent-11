...
5. **Merges v6 settings** into your existing `.claude/settings.json`:
   ...
   - **v6.2.0**: adds the **read-only quality gates** `permissions.deny` block (makes `.quality-gates.json` and `gates/` unwritable by agents; the rules use the `Edit(path)` form, which Claude Code applies to Edit, Write and MultiEdit alike, whereas `Write()`/`MultiEdit()` rule forms are silently ignored, see A11-ISS-7) and a blocking **PreToolUse "read-only gate guard"** hook (refuses Bash writes to gate paths; the guard logic lives in `.claude/hooks/gate-guard.sh`, deployed alongside)
