...
- **A single download manifest.** The mission / command / template / field-manual / skill / schema / gate / stack-profile lists moved out of function bodies into top-level `A11_*` arrays, and `--print-manifest` prints all 136 paths, including the CHANGELOG the installer reads into a variable and the rollback script URL it only prints for the user to run: a rollback command that 404s is as useless as a template that does. A path only a function body knows about is a path no test can check — which is exactly how the dead URL survived for months.

**The guard** (`project/deployment/tests/test-installer-downloads.sh`, 56 checks, exit 0): extracts the download block from `install.sh` and exercises the real functions against a genuine 404 (destination must survive), an HTML error page, an empty response, malformed JSON, unparseable bash and unparseable Python; asserts no `curl`/`wget` file write exists anywhere outside the guard; then, live, fetches all 136 manifest URLs, requires HTTP 200 for each, parses every payload by type, diffs the fetched `.mcp.json.template` against the working tree, and checks every literal call site appears in the manifest so a new download cannot be added without becoming testable. `--offline` runs the guard suite alone.

**A cold review found four more holes, all now closed.** A fresh sub-agent, given no reasoning and told to break the fix, ran the guard's own functions against adversarial payloads. It was right four times. **(1)** The error-page screen matched four literal first-line patterns, so Apache's stock `<!DOCTYPE HTML PUBLIC ...`, a leading blank line, a leading space, an XML prolog, a captive-portal `<meta refresh>` and a proxy's `Error 403:` all sailed through into the 120 `.md`/`.yaml` targets that get no type parse at all. It now takes the first 256 bytes, flattens newlines, strips leading space, lowercases, and matches the opening token, with a length-bounded rule for short proxy-error text so a document that merely discusses HTTP failures still installs. **(2)** With neither `python3` nor `jq` present the JSON fallback accepted anything starting `{`, which is the original bug reproduced verbatim on a minimal container: `{ "mcpServers": ` starts with `{`. It now balances delimiters in `awk`, and refuses outright if even `awk` is missing. **(3)** An existing `.mcp.json` was "preserved" whether or not it was valid, so damaged repos would stay damaged and the installer would keep calling 14 bytes of junk "your MCP server registry". A file that does not parse is now backed up to `.mcp.json.invalid-<timestamp>` and rebuilt from the template; a valid registry is still never touched. **(4)** A destination that was a directory made `mv` succeed by moving the file inside it, and the following `chmod 644` stripped the directory's `x` bit. Refused now, and the same shape at the other end was closed too: a `.mcp.json` that is a *directory* would have had the template copied inside it, leaving the project with no registry and no complaint. It now says so and exits 2. Three smaller ones went with them: the temp file sits beside its destination so the final rename is atomic rather than a cross-filesystem copy that can truncate; an existing destination's permissions are preserved instead of forced to 644; and `--print-manifest` no longer needs a project context or takes the install lock, so a stale lock cannot fail the release gate. Fixing that lock path exposed a real bug in the first cut: a failing command inside the `EXIT` trap aborts the trap under `set -e`, which both left `/tmp/agent11-install.lock` behind and made a completed install exit 1. Found by an end-to-end run, not by review.

**A second adversarial pass, run directly, found two more.** A payload battery against the guard's own functions caught a **UTF-8 BOM followed by `<!DOCTYPE html>`** (the BOM bytes sit in front of the first character and defeat any leading-token match) and an **S3-style `<Error><Code>NoSuchKey</Code></Error>`**, which matched none of the HTML patterns. Rather than chase each vendor's error wording, the guard now strips a BOM and rejects **any** payload whose first non-space character is `<`. That is safe because nothing this installer ships starts with an angle bracket, and the test asserts that across the whole manifest, so the day a shipped file does start with one the test fails rather than the guard silently dropping it. The same pass confirmed four behaviours hold: a symlinked destination is replaced rather than written through, so a download cannot escape the project; a read-only destination directory fails with the existing file intact and no leftovers; a real 404 leaves no `.agent11-download.*` temp beside the target; and an existing file's mode survives (600 stays 600). An unreadable junk `.mcp.json` is preserved under `.invalid-<timestamp>` and the registry rebuilt.

**A third pass, from the reviewer that had died and was relaunched, found three more — the sharpest of the lot.** **(1)** The awk JSON fallback was a bracket balancer wearing a parser's comment: it accepted `{"a": }`, `{"a" "b"}`, `{,,,}` and any prose with balanced braces, on exactly the minimal container the comment cites. It is now a real tokenising JSON walk in awk with an explicit state stack, rejecting all of those and the trailing comma, while accepting the shipped templates; the test drives 9 shapes plus both shipped JSON files through it with python3 and jq hidden from PATH. **(2)** A **dangling `.mcp.json` symlink escaped the project**: neither `-d` nor `-f` matches a link to a missing target, so the installer took the "there is no registry" branch and `cp` followed the link, creating the file **outside the repo** while reporting success. Symlinks are now handled explicitly — one pointing at valid JSON is left alone, one pointing at junk or nothing is refused rather than written through. **(3)** BSD `stat` does not follow symlinks without `-L`, so a symlinked destination returned the *link's* mode, 0755, which was then chmod'd onto the replacement: every such file came out world-executable. Worse on Linux, where GNU `stat -f` means `--file-system`, prints to stdout and exits 1, so both stat outputs concatenated and every download silently fell back to 644. Both fixed, with the result range-checked before it reaches chmod. Three smaller ones with them: an unreadable `.mcp.json` was being diagnosed as invalid and replaced, and is now refused because a file nobody has read is not a file to overwrite; the `.invalid-<timestamp>` backup could overwrite an earlier one within the same second and now takes a suffix; and the exit-2 message blamed the network for a blocked registry. The `[[ -e ]] && error` line survived only because every caller sits inside an AND-list that suspends `set -e`, so it is a plain `if` now.

**One deliberate behaviour change**: a failed `.mcp.json.template` download now exits **2** rather than 0. The squad and mission system are installed either way, so it is not a rollback, but an installer that reports success while delivering no MCP config is the same invisibility this issue is about, one layer up. A human reads the red line in the banner; a fleet script only ever reads the exit code. Documented under `--help`.

**Verification**: `bash -n` clean under bash 5 and macOS bash 3.2; shellcheck clean on the new block and the test; test suite exit 0 on five runs including from an unrelated working directory; local-mode and forced remote-mode installs into scratch directories produced a `.mcp.json.template` that `python3 -m json.tool` accepts and that `diff`s identical to the shipped template, with zero `[ERROR]` lines and no `404: Not Found` anywhere in the tree. A project seeded with the exact 14-byte junk `.mcp.json` was repaired end to end, the junk kept as `.mcp.json.invalid-<timestamp>`. The refactor was proved behaviour-neutral by running the pre-change installer and the hardened one into separate scratch projects and diffing the trees: identical, and identical again against a forced remote-mode run that pulled all 131 files over the network. `install.sh.sha256` regenerated (`secure-install.sh` verifies it).
