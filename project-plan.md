...
| **6d** | Consolidation & public comms | Single consolidated update of README, CHANGELOG (finalise `[Unreleased]` → v6.2.0), RELEASE-HISTORY, upgrade docs, and the `agent-11-website` repo — reading the User-Facing Changes running list from progress.md. Cuts the v6.2.0 release | 🔄 T1–T4 done + pushed (README v6.2.0 block, CHANGELOG `[6.2.0]`, RELEASE-HISTORY entry, UPGRADE doc; website scoped). **T5 website edit+deploy and T6 v6.2.0 tag/release await Jamie's publish confirmation** (outward-facing, per standing rule). Sprint 6 umbrella otherwise complete |
